...
double thetamax_int_x = 0.0;
double thetamin_int_x = 0.0;

double thetamax_int_y = 0.0;
double thetamin_int_y = 0.0;
...
//     Move the Y motor based on what the Y position is
    ...
    gotoLine(4);        integral = integral_x;
...
int pid_controller(double filtered, int16_t max, int16_t min, int16_t setpoint, double Kp, double Kd, double Ki, int state){
    ...
    if(state == 0 ){
        thetamax_int_x += Ki * (setpoint - max);
        thetamin_int_x += Ki * (setpoint - min);
        integral_x += Ki*error;
        
        thetamax += thetamax_int_x;
        thetamin += thetamin_int_x;
        integral = integral_x;
    }
    else {
        thetamax_int_y += Ki * (setpoint - max);
        thetamin_int_y += Ki * (setpoint - min);
        integral_y += Ki*error;
        
        thetamax += thetamax_int_y;
        thetamin += thetamin_int_y;  
        integral = integral_y;    
    }
    ...
    if (state == 0){ // X
        ...
        derivative = Kd*((error - curr_difference)/.05);
    ...
    //Shifts the values
    double top = 0.0;
    double bottom = thetamin - thetamax;
//    gotoLine(3);
//    lcd_printf("ThetaMax: %.3f   ",thetamax);
//    gotoLine(4);
//    lcd_printf("ThetaMin %.3f   ", thetamin);
...
{	
    ...
    while(1){
        ...
        if (count % 50000 == 0) {
            gotoLine(2);
            lcd_printf("%.2f, %.2f, %.3f    ", Kp_x, Kd_x, Ki_x);
            gotoLine(3);
            lcd_printf("%.2f, %.2f, %.3f    ", Kp_y, Kd_y, Ki_y);
            gotoLine(4);
            lcd_printf("SetX: %d, SetY: %d     ", x_setpoint, y_setpoint);
            gotoLine(5);
            lcd_printf("PX: %d                             ", poss_X);
            gotoLine(6);
            lcd_printf("PY: %d                             ", poss_Y);
            gotoLine(5);
            lcd_printf("PX: %d                             ", poss_X);
            gotoLine(6);
            lcd_printf("PY: %d                             ", poss_Y);
